                    ...
                    <tr>
                        ...
                        <td class="flex flex-row gap-1 items-center p-4">
                            <a href="{{ route('dashboard.akun.edit', $d->id) }}"
                                class="py-2 px-6 bg-yellow-500 hover:bg-yellow-700 rounded-2xl text-xs">Edit</a>
                            <form method="POST" action="{{ route('dashboard.akun.destroy', $d->id) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="py-2 px-6 text-xs bg-red-500 text-white rounded-2xl ">
                                    Hapus
                                </button>
                            </form>
                        </td>
