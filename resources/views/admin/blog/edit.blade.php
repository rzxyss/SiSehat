...
@section('content')
    <form method="POST" action="{{ route('dashboard.blog.update', $blog->id) }}" enctype="multipart/form-data">
        ...
        <div class="mb-6 flex flex-row justify-between px-6 gap-6">
            ...
            <div>
                <img class="h-auto" id="preview" src="{{ asset('assets/image/blog/' . $blog->thumbnail) }}"
        ...
        <div class="mb-6">
            ...
            <input type="text"
                class="py-3 px-4 text-gray-500 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0 "
                placeholder="Masukan Judul Blog" name="title" value="{{ $blog->title }}">
        </div>
        <div class="mb-6">
            <label class="block text-sm mb-2 text-gray-400">Slug</label>
            <input type="text"
                class="py-3 px-4 text-gray-500 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0 "
                name="slug" readonly value="{{ $blog->slug }}">
        </div>
        <div class="mb-6">
            <label class="block text-sm mb-2 text-gray-400">Blog</label>
            <textarea
                class="py-3 px-4 text-gray-500 block w-full border-gray-200 rounded-sm text-sm focus:border-blue-600 focus:ring-0"
                name="content" cols="30" rows="10" placeholder="Masukan Blog">
                {{ $blog->content }}
            </textarea>
        </div>
